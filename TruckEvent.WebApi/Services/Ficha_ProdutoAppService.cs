...
namespace TruckEvent.WebApi.Services
{
    ...
    {
        private readonly IFicha_ProdutoRepository _ficha_ProdutoRepository;
        public Ficha_ProdutoAppService()
        {
            this._ficha_ProdutoRepository = new Ficha_ProdutoRepository();
        }

        public Ficha_ProdutoViewModel Atualizar(Ficha_ProdutoViewModel ficha_ProdutoViewModel)
        {
            var ficha_ProdutoDTO = _ficha_ProdutoRepository.BuscarPorId(ficha_ProdutoViewModel.Id);
            var ficha_produto = Mapper.Map(ficha_ProdutoViewModel, ficha_ProdutoDTO);
        ...
        {
            return Mapper.Map<IEnumerable<Ficha_ProdutoViewModel>>(_ficha_ProdutoRepository.TrazerTodosAtivos().ToList());
        }

        public IEnumerable<Ficha_ProdutoViewModel> TrazerTodosDeletados()
        {
            return Mapper.Map<IEnumerable<Ficha_ProdutoViewModel>>(_ficha_ProdutoRepository.TrazerTodosDeletados().ToList());
        }
    }
}